...
class MarkerGenerator {
  final double _markerSize = 80;
  double _circleStrokeWidth = 3;
  double _circleOffset = 20;
  double _outlineCircleWidth = 50;
  double _fillCircleWidth = 50;
  double _iconSize = 200;
  double _iconOffset = 200;
  ...
  Future<BitmapDescriptor> createBitmapDescriptorFromText(String text, Color iconColor, Color circleColor, Color backgroundColor) async {
    final pictureRecorder = PictureRecorder();
    final canvas = Canvas(pictureRecorder);

    _paintCircleFill(canvas, backgroundColor);
    _paintCircleStroke(canvas, circleColor);
    _paintText(canvas, iconColor, text);
  ...
  void _paintText(Canvas canvas, Color color, String text) {
    final textPainter = TextPainter(textDirection: TextDirection.ltr);
    textPainter.text = TextSpan(
        text: text,
        style: TextStyle(
    ...
    textPainter.layout();
    textPainter.paint(canvas, Offset(_iconOffset + 10, _iconOffset -5));
  }
